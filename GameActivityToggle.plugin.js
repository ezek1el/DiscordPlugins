...
class GameActivityToggle {
	...
	getName() {return "Game Activity Toggle";}

	getDescription() {return 'Simple plugin that adds a "Display Game Activity" button on the main toolbar so you can toggle it easier.';}

	getVersion() {return "1.2.7";}

	getAuthor() {return "egrodo";}
	...
	start() {
		...
		/* Added a little timeout so it doesnt get interfeared by other plugins */
	...
	createButton() {
		...
		/* Ignore pointer events to fix bug that was causing repeated clicks to be ignored. */
	...
	onButtonMouseOver({ target }) {
		...
		/* 8 being a constant amount of space to hover above the btn. */
		const tooltipYPos = y - target.clientHeight - 8; 
